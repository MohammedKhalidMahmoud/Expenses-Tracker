...
function add_expense(e) {
    ...
    console.log(data);
    axios.post('http://localhost:3000/api/expens', data)
    // fetch('http://localhost:3000/api/expens', {
    //     method: 'POST',
    //     headers: {
  ...
  return (
    ...
    <form className="max-w-md mx-auto mt-10">
        ...
        <div className="relative z-0 w-full mb-5 mt-10 group">
            <input value={amount} onChange={(e)=>{setAmount(e.target.value)}} type="number" name="floating_amount" id="floating_amount" className="input_field peer" placeholder=" " required />
        ...
        <div className="relative z-0 w-full mb-5 group">
            <input value={category} onChange={(e)=>{setCategory(e.target.value)}} type="password" name="floating_password" id="floating_password" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
            <label htmlFor="floating_password" className="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Password</label>
        </div>
        <div className="relative z-0 w-full mb-5 group">
            <input value={date} onChange={(e)=>{setDate(e.target.value)}} type="password" name="repeat_password" id="floating_repeat_password" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
