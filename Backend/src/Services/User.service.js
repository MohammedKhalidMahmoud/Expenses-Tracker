...
export async function getUsers({ page = 1, limit = 10 }) {    // paginated version
  if (limit === 0) {
    return User.findAndCountAll({attributes:['id','name','email','createdAt', 'updatedAt']}); // fetch all
  }
...
export function deleteUserById(id) {
    return User.destroy({ where: { id }, attributes:['id','name','email','isActive','createdAt','updatedAt'] });
}
